...
def locatorOnSelection( ro = 'zxy', X = 1.0, constrain = True, toSelection = False, color = 18, matchSet = False, group = False ):
    sel = cmds.ls( sl = True )
    # print( sel )
    grps = []
    locs = []
    ...
    if group:
        for item in locs:
            grp = plc.null2( item.replace( 'PLACE', 'GRP' ), item, orient = True )
            grps.append( grp )
