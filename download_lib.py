...
def shelfBuild( *args ):
    ...
    if not cmds.control( dp.shelf, q=1, ex=1 ):
        ...
        wh = 35
        #build buttons
        cmds.shelfButton( label='FRSH', annotation='refresh AnimGit shelf', w=wh, h=wh, image='dwnArrow.xpm', command='import download_lib as dl\nreload(dl)\ndl.shelfRefresh(getAll=1)' )
        cmds.shelfButton( label='save ++', annotation='save ++', w=wh, h=wh, image='save++_icon.xpm', command='import fileMan_lib as fm\nreload(fm)\nfm.incrementalSave()' )
        cmds.shelfButton( label='', annotation='set project from file name', w=wh, h=wh, image='PrjtSet.xpm', command='import fileMan_lib as fm\nreload(fm)\nfm.setProjectFromFilename("scenes")' )
        ...
        cmds.shelfButton( label='anim is imported from a file', annotation='anim is imported from a file', w=wh, h=wh, image='AnimImport.xpm', command='import clipPickle_lib as cp\nreload(cp)\ncp.clipApply()' )
        cmds.shelfButton( label='anim is exported to a file', annotation='anim is exported to a file', w=wh, h=wh, image='AnimExport.xpm', command='import clipPickle_lib as cp\nreload(cp)\ncp.clipSave()' )
