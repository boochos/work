...
def get():
    ...
    if not cmds.control(shelf, q=1, ex=1):
        ...
        cmds.shelfButton(annotation='speed attribute is added\nneeds to updated to accomodate scale settings for projects', w=wh,h=wh, image='kmh.xpm', command='import display_lib as ds\nreload(ds)\nds.speed(local=0)' )
        cmds.shelfButton(annotation='select 2 objects\ndistance attribue is added\nattr only updates on frame changes\nif attr exists it is removed', w=wh,h=wh, imageOverlayLabel='dst', image='pythonFamily.png', command='import display_lib as dis\nreload(dis)\ndis.distance()' )
