...
def get():
    ...
    if not cmds.control(shelf, q=1, ex=1):
        ...
        cmds.shelfButton(label='anim is imported from a file', annotation='anim is imported from a file', w=wh,h=wh, image='AnimImport.xpm', command='import animation_library_manager as animMan\nreload(animMan)\nanimMan.animImport()' )
        cmds.shelfButton(label='anim is exported to a file', annotation='anim is exported to a file', w=wh,h=wh, image='AnimExport.xpm', command='import animation_library_manager as animMan\nreload(animMan)\nanimMan.animExport()' )
