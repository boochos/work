...
def proxyGeo(name, joint, width, height, ty=0, tz=0, mlt=1, orient=True):
    ...
    ##attributes
    misc.hijackAttrs(geoConnections, jointsGpOffset, 'width', 'width', set=False, default=width)
    misc.hijackAttrs(geoConnections, jointsGpOffset, 'height', 'height', set=False, default=height)
    misc.hijackAttrs(geoConnections, jointsGpOffset, 'depth', 'depth', set=False, default=tz)
    ...
    if type(joints) == list:
        ...
        for item in joints:
            if item != joints[len(joints)-1]:
                ...
                distance = misc.distance2Pts(pos1, pos2)
                if distance != 0:
                    ##create nulls
                    ...
                    misc.hijackAttrs(geoConnections, jointsGpOffset, 'width', 'width', set=False, default=width)
                    misc.hijackAttrs(geoConnections, jointsGpOffset, 'height', 'height', set=False, default=height)
                    misc.hijackAttrs(geoConnections, jointsGpOffset, 'depth', 'depth', set=False, default=distance)
