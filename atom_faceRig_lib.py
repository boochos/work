...
def splineSettings( prefix, Vis = 1, Root = 0, Stretch = 0,
    ...
    cmds.setAttr( prefix + '_IK_CtrlGrp.' + 'Vis', Vis )
    cmds.setAttr( prefix + '_IK_CtrlGrp.' + 'Root', Root )
    cmds.setAttr( prefix + '_IK_CtrlGrp.' + 'Stretch', Stretch )
...
def spline( SPLN_Name = 'SPLN', SPLN_Size = 0.5, SPLN_Dist = 3.0, SPLN_Falloff = 0, SPLN_Prnt = ['splinePRNT', 'startPRNT', 'endJPRNT'],
    ...
    '''
    face = None
    X = 1  # cmds.floatField( 'atom_srig_conScale', query = True, value = True )
...
def snakeTongue( Name, X ):
    ...
    tngTipCt = place.Controller2( Name + 'Tip', 'tongue_05_jnt', True, 'facetZup_ctrl', X * 2.5, 12, 8, 1, ( 0, 0, 1 ), True, True, colorName = 'yellow' ).result
    ...
    spline( SPLN_Name = Name + 'Base', SPLN_Size = X * .6, SPLN_Dist = 2, SPLN_Falloff = 1,
           ...
           SPLN = ['tongue_01_jnt', 'tongue_05_jnt'],
