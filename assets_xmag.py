...
def puck():
    ...
    # puck #
    Puck = 'puck'
    puck = place.Controller( Puck, MasterCt[0], False, 'loc_ctrl', X * 2, 17, 8, 1, ( 0, 0, 1 ), True, True )
    PuckCt = puck.createController()
    place.setRotOrder( PuckCt[0], 2, True )
    cmds.parent( PuckCt[0], CONTROLS )
    cmds.setAttr( PuckCt[0] + '.translateY', -1.267 )
    ...
    cmds.setAttr( Puck_u_Ct[0] + '.translateZ', 1.267 * 3 )
    ...
    cmds.setAttr( Puck_t_Ct[0] + '.translateY', 1.267 )
