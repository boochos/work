...
def message( what = '', warning = False ):
    ...
    if warning:
        print( '  WARNING  ' + what )
        cmds.ui.displayMessage( '  WARNING  ' + what )
    else:
        print( what )
        # cmds.ui.displayMessage( what )
    ...
    def _pets( self, e, directory_only = False ):
        ...
        if item:
            ...
            if setProject_window.scenes.selectedItems():
                ...
                self.suffix = filename_getSuffix( path = self._p, entity = self._e, task = self. )
            #
    ...
    def _path( self, directory_only = False ):
        ...
        if self._selected == self._p:
            ...
            if 'asset' in self._e:
                # self.path = os.path.join( self.root, self._p, self._e, self._t )
                if directory_only:
                    self.path = os.path.join( self.root, self._p, self._e, self._t, 'houdini' )
                else:
                    self.path = os.path.join( self.root, self._p, self._e, self._t, 'houdini', self._s )
            else:
                if directory_only:
                    self.path = os.path.join( self.root, self._p, self._e, self._t, 'houdini' )
                else:
...
def init_ui():
    ...
    tag_button.setToolTip( "Houdini session" )
    color = [0.5, 0.5, 0.5]
    # color = get_tag_color()
    ...
    #
    create_button = QtWidgets.QPushButton( "Sanitize Name + Save Version" )
    ...
    set_open_button.setStyleSheet( "padding-top: " + pad + "px; padding-bottom: " + pad + "px; background-color: blue;" )
    # set_open_button.setStyleSheet( "background-color : green" )
    ...
    '''
    ver_button = QtWidgets.QPushButton( "Sanitize Version + Save" )
    ver_button.setStyleSheet( "padding-top: " + pad + "px; padding-bottom: " + pad + "px;" )
    ver_button.clicked.connect( lambda: save_plus( scene_list_widget, project_list_widget, shots_and_assets_list_widget, tasks_list_widget ) )
    ...
    col4_layout.addWidget( ver_button )
...
def get_scenes( list_widget = None, project = None, entity = None, task = None ):
    ...
    if task[:6] == 'assets':  # alembics
        ...
        raw_scene_directory = os.path.join( os.environ['PROJ_ROOT'], project, entity, task, 'houdini' )
        # qualify
...
def set_project( project, entity, task, scene, opn = False, create = None, suffix_edit = None ):
    ...
    project_path = False
    if go:
        project_dir = os.path.join( os.environ["PROJ_ROOT"], project_txt, entity_txt, task_txt, 'houdini' ).replace( '\\', '/' )
        ...
        if opn:
            if scene_txt:
                if task_txt[:6] != 'assets':  # alembics:
                    pth = os.path.join( os.environ["PROJ_ROOT"], project_txt, entity_txt, task_txt, 'houdini', scene_txt )
                    changes = cmds.hipFile.hasUnsavedChanges()
                    if changes:
                        # print( 'changes' )
                        open_prompt( path = pth )
                    else:
                        # print( 'no changes, here ', pth )
                        cmds.hipFile.load( pth, suppress_save_prompt = True )
                        cmds.putenv( 'JOB', project_dir )  # reset to verride whatever is in the file
                else:
                    message( 'Alembic cannot be opened. Only imported. Currently unsupported.', warning = 1 )
            else:
                '''
                # cant find an open file dailgue command in houdini
                pth = cmds.fileDialog2( fileMode = 1, startingDirectory = os.path.join( os.environ["PROJ_ROOT"], project_txt, entity_txt, task_txt, 'houdini' ) )
                if pth:
                    cmds.file( pth, open = True, force = False )'''
                message( 'Select a file', warning = 1 )
        if create: # save as
            if project_path:
                pth = os.path.join( os.environ["PROJ_ROOT"], project_txt, entity_txt, task_txt, 'houdini' )
                result = None
                if suffix_edit.text(): # check suffix field for text
                    sfx = suffix_edit.text()
                    result = filename( pth, entity_txt, task_txt + '_' + sfx ) # save
                else:
                    sfx = filename_getSuffix( pth, entity_txt, task_txt ) # check filename for suffix, func only uses task_txt
                    if sfx:
                        result = filename( pth, entity_txt, task_txt + '_' + sfx ) # save
                    else:
                        result = filename( pth, entity_txt, task_txt ) # save
                if result:
                    # print( project, entity )
                    get_scenes( refresh_scenes[0], project, entity, refresh_scenes[3] ) # refresh ui
            else:
...
def explore_path( project, entity = None, task = None, scene = None ):
    ...
    if task:
        if task.selectedItems():
            task = task.selectedItems()[0].text()
            path = os.path.join( path, task, 'houdini' )
    # open file browser
...
def filename_getSuffix( path = '', entity = '', task = '' ):
    '''
    this does not look very robust
    '''
    suffix = ''
    # s_path = cmds.file( query = True, exn = True )
    s_path = cmds.hipFile.basename()
    print( s_path )
    if s_path:
        if task in s_path:  #
            if s_path[-8:] != 'untitled':
            ...
            message( 'Sanitizing unconventional name: ' + s_path, warning = True )
    return suffix
...
def filename( path, entity, task ):
    ...
    if e:
        e = e[-1]
    fl = e + '_' + task + '_v001'
    result = incrementalSave( path, fl, 'hip' )
    return result
...
def incrementalSave( basepath, basename, extension ):
    ...
    if scene_info is not False:
        ...
        message( name )
        cmds.hipFile.setName( name )
        # save it
        cmds.hipFile.setSaveMode( cmds.saveMode.Text )  # as text
        cmds.hipFile.save( save_to_recent_files = True )
        return True
...
def parseSceneFilePath( path ):
    ...
    if not len( path ):
        ...
        if path.rfind( '.' ) > -1:
            # split the scene file name apart
...
def rangeFromMaFile( project, entity, task, scene, handles = 0 ):
    ...
    if go:
        try:
            ...
            path = os.path.join( os.environ["PROJ_ROOT"], project, entity, task, 'houdini', scene )
            play_start = 0
            play_end = 0
            all_start = 0
            all_end = 0
            # print path
            if os.path.isfile( path ) and '.hip' in path:
                inFile = open( path, 'r' )
                for line in inFile.readlines():
                    ...
                    if 'tset' in line:
                        ...
                        #
                        all_start_t = float( line_parts[1] )
                        all_start = cmds.timeToFrame( all_start_t )
                        #
                        all_end_t = float( line_parts[2] )
                        all_end = cmds.timeToFrame( all_end_t ) - 1  # remove one, cuz houdini
                        # print( play_start )
                        # print( play_end )
                        # print( all_start )
                        # print( all_end )
                        if handles:
                            cmds.playbar.setFrameRange( all_start, all_end )
                            cmds.playbar.setPlaybackRange( all_start + handles, all_end - handles )
                        else:
                            cmds.playbar.setFrameRange( all_start, all_end )
                            cmds.playbar.setPlaybackRange( all_start, all_end )
                        inFile.close()
                        return None
                inFile.close()
                return None
            else:
                message( 'Select a .hip file', warning = 1 )
                # print 'not a directory'
        except:
            message( 'ERROR, file likely not saved as text', warning = True )
    else:
        message( 'Select a .hip file', warning = 1 )
...
def fpsFromMaFile( project, entity, task, scene ):
    ...
    if not go:
        ...
        return
    # print( go )
    all_start = cmds.playbar.frameRange()[0]
    all_end = cmds.playbar.frameRange()[1]
    play_start = cmds.playbar.playbackRange()[0]
    play_end = cmds.playbar.playbackRange()[1]
    fps = ''
    if project.selectedItems():
        project = project.selectedItems()[0].text()
    else:
    ...
    if go:
        ...
        try:
            ...
            if os.path.isfile( path ) and '.hip' in path:
                ...
                for line in inFile.readlines():
                    cvLine = line.strip( '\n' )
                    if 'fps' in line:  # currentUnit -l centimeter -a degree -t film;
                        # print( line )
                        fps = float( line.split( ' ' )[1] )
                        print( fps )
                        old_fps = cmds.fps()
                        cmds.setFps( fps = fps, modify_frame_count = False, preserve_keyframes = True, preserve_frame_start = True )
                        # reset range
                        cmds.playbar.setFrameRange( all_start, all_end )
                        cmds.playbar.setPlaybackRange( play_start, play_end )
                        # exit
                        inFile.close()
                        message( 'old fps: ' + str( old_fps ) + '  ---  new fps: ' + str( fps ) )
                        return None
                inFile.close()
                return None
            else:
    ...
    if go:
        ...
        if scene:
            if task[:6] == 'assets':  # alembics
                ...
                pth = os.path.join( os.environ["PROJ_ROOT"], project, entity, task, 'houdini', scene )
...
def save_open_scene( main_window, path = '' ):
    ...
    '''
    cmds.hipFile.save( save_to_recent_files = True )
    cmds.hipFile.load( path, suppress_save_prompt = True, ignore_load_warnings = True )
...
def new_scene_detect():
    ...
    if changes:
        ...
        cmds.hipFile.clear( suppress_save_prompt = True )
        cmds.hipFile.setName( 'untitled.hip' )
...
def save_plus( scenes, project, entity, task ):
    '''
    strictly versions file
    '''
    # parse file name
    scene = cmds.hipFile.name()
    print( 'path: ', scene )
    scene_info = parseSceneFilePath( scene )
    print( 'scene_info: ', scene_info )
    version_prefix = scene_info[1][-5:-3]  # if proper naming convention this should be '_v'
    # print( 'prefix', version_prefix )
    current_version = scene_info[1][-3:]
    # print( current_version )

    # qualify version number and convention
    convention = False
    try:
        v = int( current_version )
        if v and version_prefix == '_v':
            convention = True
    except:
        # print( 'no version' )
        pass
    ...
    # create version convention
    if not convention:
        current_version = '001'
        scene_info[1] = scene_info[1] + '_v'
    # return
    # make sure the correct file format is being used
    if scene_info is not False:
        # if no number was found, force it to 000
        # find the highest version of the current scene name in the folder, then increment the current scene to one more than that then save
        files = os.listdir( scene_info[0] )
        num = int( current_version ) - 1
        string_info = splitEndNumFromString( scene_info[1] )
        ...
        for f in files:
            # make sure the current file is not a directory
            if os.path.isfile( os.path.join( scene_info[0], f ) ):
                # remove any file that has a '.' as the first letter
                if f[0] != '.':
                    phile = parseSceneFilePath( os.path.join( scene_info[0], f ) )
                    if phile is not False:
                        file_info = splitEndNumFromString( phile[1] )
        ...
        name = name.replace( '\\', '/' )
        # get the current file type
        fileType = cmds.hipFile.saveMode()
        print( 'mode', fileType )
        # rename the current file
        cmds.hipFile.setName( name )
        # save it
        ...
        # refresh  list_widget = None, project = None, entity = None, task = None
        get_scenes( list_widget = scenes, project = project, entity = entity, task = task )
...
def get_project( path = '' ):
    '''
    parse entire scene path, return only project portion
    '''
    print( path )
    print( path.rsplit( '\\', 1 ) )
    prj = path.rsplit( '\\', 1 )[0]
...
class Prefs():
    ...
    def prefPath( self, *args ):
        varPath = cmds.getenv( 'HOME' )  # user
        path = os.path.join( varPath, 'houdini/scripts/python' )
