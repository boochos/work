...
def getThumb( filein, fileout, delete = False, scale = 1.0 ):
    ...
    if delete:
        ...
        if create:
            ...
            # C:\Users\sebas> PowerShell -Command "$FilePath='C:\Users\sebas\t.mov'; Write-Host ((New-Object -COMObject Shell.Application).NameSpace((Split-Path -Parent -Path $FilePath))).ParseName((Split-Path -Leaf -Path $FilePath)).ExtendedProperty('System.Video.FrameWidth')"
            try:
                ff = locateffmpeg()
