...
def findControl( ann = '', panelTyp = '', split = 3 ):
    ...
    result = []
    for con in cntrls:
        if cmds.control( con, exists = True ):
...
def geField( name = '', parent = '', attach = None, label = '', cmd = '', w = 100, gap = 10, tx = '', ann = '' ):
    if not cmds.control( name, ex = 1 ):
        ...
        field = cmds.textField( name, tx = tx, h = 21, w = w, ec = cmd, ann = ann, aie = True )  # no change command
