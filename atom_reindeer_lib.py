...
def preBuild(
    ...
    PreBuild    = misc.rigPrebuild(Top=0, Ctrl=True, SknJnts=True, Geo=True, World=True, Master=True, OlSkool=True, Size=X*180)
...
def buildAppendages(*args):
    ...
    misc.shapeSize('Back_digit_1_ctrl_L', digX)
    misc.shapeSize('Back_digit_2_ctrl_L', digX)
    ...
    misc.shapeSize('Back_digit_1_ctrl_R', digX)
    misc.shapeSize('Back_digit_2_ctrl_R', digX)
    ...
    misc.shapeSize('Front_digit_1_ctrl_L', digX)
    misc.shapeSize('Front_digit_2_ctrl_L', digX)
    ...
    misc.shapeSize('Front_digit_1_ctrl_R', digX)
    misc.shapeSize('Front_digit_2_ctrl_R', digX)
...
def harness():
    ...
    def surfJnts(name, pairL):
        ...
        for pair in pairL:
            ...
            iks.append(ikhndl)
            #cleanup
            misc.cleanUp(jnts[0], SknJnts=True)
            misc.cleanUp(jnts[2], SknJnts=True)
            misc.cleanUp(ikhndl, World=True)
        return jntPairs, iks
        ...
        for vSet in vertSets:
            ...
            for point in vSet:
                if i < 3:
                    ...
                    misc.cleanUp(DiamondCt[0], Ctrl=True)
