...
def createVerticalScapRig( shoulder, shoulder_dbl, shoulder_grp, scapula, control, spine, suffix, setChannels = False, moveAxis = 'translateX', flip = False, driverAxis = 'translateZ', amount = 0.4 ):
    '''
    ...
    if flip:
        cmds.setAttr( mult + '.input2Y', amount * -1 )
    else:
        cmds.setAttr( mult + '.input2Y', amount )
    cmds.connectAttr( control + '.' + driverAxis, mult + '.input1Y' )
...
def create_3_joint_pv2( stJnt = '', endJnt = '', prefix = '', suffix = '', distance_offset = 0.0, orient = True, color = 'yellow', X = 1, midJnt = '' ):
    ...
    ***
    ...
    #  B /_|_| d is the length from A to D. (not half of b), solve d (object space translateZ move)
    #    \   | e is the length from D to B, solve e (object space translateY move)
    #     \  |
    #     a\ |
    #       \|
    #        C
    ...
    # C = end joint
    #
    # algo creates loc at point_A (aimed at B, y up), rotates using angle A (should point at point_D), moves d distance (should be at point_D), move e distance (should be at point_B)
    #
    if not midJnt:
        midJnt = joint.jointTravers( stJnt, 1 )
    # z points down chain
...
def createReverseAnkle( prefix, suffix, name, ankleJnt, paw_fk, aimAxisList, upAxisList, flipVal, setChannels = True, color = 17 ):
    ...
    right_side = False
    if cmds.xform( paw_fk, q = True, ws = True, t = True )[0] < 0:  # if world space is negative tx, right limb, flip certain controls
        right_side = True
    ...
    ankle_up_loc = place.circle( place.buildName( prefix, suffix, '_ankle_aim_ctrl' ), ankle_up_grp, 'loc_ctrl', X * 1, color, 8, 1, [0, 1, 0] )
    cmds.parent( ankle_up_loc, ankle_up_grp )

    # place.null2( 'step_1', ankle_up_loc, orient = True )

    cmds.xform( ankle_up_grp, ws = True, t = ankle_pos )
    # place.null2( 'step_2', ankle_up_loc, orient = True )
    ...
    # place.null2( 'step_4', ankle_aim_loc[0], orient = True )
    ...
    if len( sel ) != 0:
        if cmds.window( 'atom_win', ex = True ):
            ...
            if cmds.nodeType( hipJnt ) == 'joint':
                ...
                if right_side:
                    cmds.xform( heelRollCtrl, r = True, os = True, ro = [ 0, 180, 180 ] )
                # '''
                # new end
                # return None
                ...
                ballRollCtrl_Grp = place.insert( 'null', 1, ballRollCtrl[0] + '_Grp' )  # place above
                if right_side:
                    cmds.xform( ballRollCtrl_Grp[0], r = True, os = True, ro = [ 0, 180, 180 ] )
                ...
                for i in range( 0, len( ankleChildren ), 1 ):
                    ...
                    if joint.jointTravers( ankleChildren[i], 1 ):
                        ...
                        if child != vol_metacarpal:
                            ...
                            # meta ik
                            meta_ik = create_ik( metacarpal, proximal_phalanx, prefix, suffix, 'metacarpal_' + str( cnt ), None, False, setChannels, color = color )  # this should be swapped out for aim constraint, 2 joint ik likes to flip
                            ...
                            # '''
                            #
                            if right_side:  # if world space is negative tx, right limb
                                #
                                cmds.xform( baseCtrlGrp, ws = True, ro = pp_rot )
                                # cmds.xform( baseCtrlGrp, r = True, os = True, ro = [0, 180, 0] ) commented out for biped build, replaced with bottom line
                                cmds.xform( baseCtrlGrp, r = True, os = True, ro = [ 0, 180, 180 ] )
                            else:
                                cmds.xform( baseCtrlGrp, ws = True, ro = pp_rot )
                ...
                ankle_aim_loc = createReverseAnkle( prefix, suffix, limbName, ankleJnt, paw_fk, aimAxisList, ankleUpAxis, flipVal, setChannels, color )
                # return None
                ...
                if right_side:
                    cmds.xform( ankleFromToeRollGrp, r = True, os = True, ro = [ 0, 180, 180 ] )
                cmds.parent( ankleFromToeRollGrp, limbCtrl_grp )
                ...
                # cmds.xform( autoAnkleUp_loc[0], r = True, t = [hipJntPos[0], hipJntPos[1], hipJntPos[2] + 15] )  # old
                cmds.xform( autoAnkleUp_loc[0], r = True, t = [hipJntPos[0] + ankleUpAxisList[0], hipJntPos[1] + ankleUpAxisList[1], hipJntPos[2] + ankleUpAxisList[2]] )  # new, but should reflect the direction of skeleton. not exactly sure what yet
                ...
                '''
                ankleCtrl = place.circle( name = place.buildName( prefix, suffix, limbName + '_ankle_ctrl' ), obj = ankleJnt, shape = 'loc_ctrl', size = X * 5, color = color, sections = 8, degree = 1, normal = ( 0, 0, 1 ), orient = True )
                ...
                # pvRig( name, Master, Top, Btm, Twist, pv, midJnt, X, slider, setChannels = True, up = [1, 0, 0], aim = [0, -1, 0], color = 17 )
...
def createThighTwistRig( suffixList, w = -0.75 ):
    for side in suffixList:
        # parentForarmRigJoints( 'leg_hip_jnt_%s' % side, 'leg_hip_twist', side )
        cmds.parent( 'leg_hip_twist_jnt_' + side, 'leg_hip_jnt_' + side )

        # front_twist_aim_loc_L
        rootMd = cmds.createNode( 'multiplyDivide', name = 'leg_hip_twist_MD_%s' % side )
        cmds.setAttr( rootMd + '.input2Z', w )

        # make the connections
        cmds.connectAttr( 'leg_hip_jnt_%s.rotateZ' % side, rootMd + '.input1Z' )
        cmds.connectAttr( rootMd + '.outputZ', 'leg_hip_twist_jnt_%s.rotateZ' % side )
...
def createDeltTwistRig( suffixList, w = -0.75 ):
    ...
    for side in suffixList:
        ...
        cmds.parent( 'arm_shoulder_twist_jnt_' + side, 'arm_shoulder_jnt_' + side )

        # front_twist_aim_loc_L
        rootMd = cmds.createNode( 'multiplyDivide', name = 'arm_shoulder_twist_MD_%s' % side )
        cmds.setAttr( rootMd + '.input2Z', w )

        # make the connections
        cmds.connectAttr( 'arm_shoulder_jnt_%s.rotateZ' % side, rootMd + '.input1Z' )
        cmds.connectAttr( rootMd + '.outputZ', 'arm_shoulder_twist_jnt_%s.rotateZ' % side )


def createWristTwistRig( suffixList, w = -0.75 ):
    '''
    driven twist joint
    driver joint
    final parent
    ...
    for side in suffixList:
        # parentForarmRigJoints( 'front_shoulder_jnt_%s' % side, 'delt_twist', side )
        cmds.parent( 'arm_wrist_twist_jnt_' + side, 'arm_elbow2_jnt_' + side )

        # front_twist_aim_loc_L
        rootMd = cmds.createNode( 'multiplyDivide', name = 'arm_wrist_twist_MD_%s' % side )
        cmds.setAttr( rootMd + '.input2Z', w )
...
def createButtVolumeRig( suffixList, w = 0.1 ):
    '''
    assumes one joint sits on top the other, connection reduces rotations by given amount
    '''
    for side in suffixList:
        # parentForarmRigJoints( 'leg_hip_jnt_%s' % side, 'hip_volume', side )
        # cmds.parent( 'hip_volume_jnt_' + side, 'leg_hip_jnt_' + side )

        # front_twist_aim_loc_L
        rootMd = cmds.createNode( 'multiplyDivide', name = 'leg_butt_volume_MD_%s' % side )
        cmds.setAttr( rootMd + '.input2X', w )
        cmds.setAttr( rootMd + '.input2Y', w )
        cmds.setAttr( rootMd + '.input2Z', w )

        # make the connections
        cmds.connectAttr( 'leg_hip_jnt_%s.rotateX' % side, rootMd + '.input1X' )
        cmds.connectAttr( rootMd + '.outputX', 'leg_hip_volume_jnt_%s.rotateX' % side )
        #
        cmds.connectAttr( 'leg_hip_jnt_%s.rotateY' % side, rootMd + '.input1Y' )
        cmds.connectAttr( rootMd + '.outputY', 'leg_hip_volume_jnt_%s.rotateY' % side )
        #
        cmds.connectAttr( 'leg_hip_jnt_%s.rotateZ' % side, rootMd + '.input1Z' )
        cmds.connectAttr( rootMd + '.outputZ', 'leg_hip_volume_jnt_%s.rotateZ' % side )


def createDeltVolumeRig( suffixList = ['L', 'R'], w = 0.25 ):
    '''
    assumes one joint sits on top the other, connection reduces rotations by given amount
    '''
    for side in suffixList:
        # parentForarmRigJoints( 'front_shoulder_jnt_%s' % side, 'delt_twist', side )
        # cmds.parent( 'shoulder_volume_jnt_' + side, 'arm_shoulder_jnt_' + side )

        # front_twist_aim_loc_L
        rootMd = cmds.createNode( 'multiplyDivide', name = 'arm_shoulder_volume_MD_%s' % side )
        cmds.setAttr( rootMd + '.input2X', w )
        cmds.setAttr( rootMd + '.input2Y', w )
        cmds.setAttr( rootMd + '.input2Z', w )

        # make the connections
        cmds.connectAttr( 'arm_shoulder_jnt_%s.rotateX' % side, rootMd + '.input1X' )
        cmds.connectAttr( rootMd + '.outputX', 'arm_shoulder_volume_jnt_%s.rotateX' % side )
        #
        cmds.connectAttr( 'arm_shoulder_jnt_%s.rotateY' % side, rootMd + '.input1Y' )
        cmds.connectAttr( rootMd + '.outputY', 'arm_shoulder_volume_jnt_%s.rotateY' % side )
        #
        cmds.connectAttr( 'arm_shoulder_jnt_%s.rotateZ' % side, rootMd + '.input1Z' )
        cmds.connectAttr( rootMd + '.outputZ', 'arm_shoulder_volume_jnt_%s.rotateZ' % side )
