...
def createVerticalScapRig(shoulder, shoulder_dbl, shoulder_grp, scapula, control, spine, suffix, setChannels=False):
    ...
    cmds.connectAttr(control + '.translateZ', mult + '.input1Y')
...
def stepToGoal(goal, sample, obj, operation, goalAxis, axis, uiFlip):
    goal = goal[goalAxis]
    ...
    if len(sel) !=0:
		...
		ankle_pv_loc = create_3_joint_pv(hipJnt, ankleJnt, prefix, suffix, limbName, 'atom_qls_limbRot_radioButtonGrp', 'atom_qls_limbAim_radioButtonGrp',
                ...
                for i in range(0, len(ankleChildren),1):
                    ...
                    if joint.jointTravers(ankleChildren[i],1) != None:
		...
		cmds.connectAttr(sel[2] + '.AnkleUp', ankleShape + '.visibility')
                ...
                stepToGoal(cmds.xform(hipJnt,query=True,ws=True,rp=True), .001, ankleFromToeRollGrp_aim_loc, '<', 1, aimAxisList,flipVal)
		...
		misc.hijack(ankleIkParentGrp, ballRollCtrl[0], rotate=False, scale=False, visibility=False)
                ...
                ankleTwist(misc.buildName(prefix, suffix, '_ankle_control_grp'), 'master_Grp', hip_ctrl, ankleToToeRollGrp,
...
def createForarmTwistRig(suffixList):
