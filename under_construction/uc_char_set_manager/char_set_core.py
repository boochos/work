    ...
    if characterMembers is not None:
        ...
        for cMember in characterMembers:
            if cmds.nodeType( cMember ) != 'character':
                data['members'].append( {'attribute': cMember} )
            else:
                charList.append( cMember )
        for char in charList:
            data['members'].append( collectData( char, parent ) )
    return data


def exportFileToJSON( selChar, filePath ):
    """ 
    # Example usage
    # exportFileToJSON('yourCharacterSet', 'outputFilePath.json')
    """
    ...
    if cmds.nodeType( selChar ) == 'character':
        if os.path.isdir( filePath ):
            ...
            data = collectData( selChar, 'none' )
            with open( filePath, 'w' ) as outFile:
                json.dump( data, outFile, indent = 4 )
            message( "File '{}' saved".format( filePath ) )
    else:
        message( 'Object ::{}:: is not a characterSet.'.format( selChar ) )


def processCharacterSet( data, parentCharName, prefix = '', ns = '', rp = None ):

    for key, value in data.items():
        if key == 'character':
            charName = prefix + value

            # Create character set or sub-character set
            if parentCharName == 'none' or parentCharName == '':
                charName = cmds.character( name = charName, em = True )
            else:
                print( charName, parentCharName )
                charName = cmds.character( name = charName, em = True )
                cmds.character( charName, fe = parentCharName )
        elif key == 'members':
            for member in value:
                if 'attribute' in member:
                    attributeName = member['attribute']
                    if ns:
                        attributeName = updateNS( attributeName, ns )
                    if rp:
                        attributeName = replaceInString( attributeName, rp )
                    if not cmds.objExists( attributeName ):
                        sel = cmds.ls( sl = True )
                        if len( sel ) > 0:
                            sel = sel[0]
                            sel = nameSpace( sel, base = True )
                            attributeName = updateNS( attributeName, sel[0] + ':' + sel[1] )
                        else:
                            message( 'Object ' + attributeName + ' was not found. Select an object in desired namespace and try again.' )
                    try:
                        cmds.character( attributeName, fe = charName )
                    except:
                        loadWarning = 'No attribute found, {}, skipping...'.format( attributeName )
                        mel.eval( "warning\"{}\";".format( loadWarning ) )
                    else:
                        cmds.character( attributeName, fe = charName )
                else:
                    # Process nested character sets
                    processCharacterSet( member, charName, prefix, ns, rp )


def importFileFromJSON( path = '', prefix = '', ns = '', rp = None ):
    if os.path.isfile( path ):
        with open( path ) as file:
            data = json.load( file )
            processCharacterSet( data, 'none', prefix, ns, rp )
    else:
