...
def getMapPnt(anchorList, cv, printInfo=False):
    ...
    for mic in anchorList:
        getDistance = aml.distance2Pts(cv.pos, cmds.xform(mic, q=True, ws=True, t=True))
    ...
    for geo in OldSkooObj.meshList:
        ...
        for i, vert in enumerate(verts):
            getDistance = aml.distance2Pts(cv.pos, cmds.xform(vert,q=True, ws=True, t=True))
            if getDistance < distance:
                distance     = getDistance
                closestPoint = vert
                closestObj   = geo
    ...
    if distance > .005:
        for crv in OldSkooObj.curveList:
            ...
            for i in range(0,points,1):
                cvStr       = crv + '.cv[%d]' % i
                getDistance = aml.distance2Pts(cv.pos, cmds.xform(cvStr, q=True, ws=True, t=True))
                if getDistance < distance:
                    distance     = getDistance
                    closestPoint = cvStr
                    closestObj   = i  
