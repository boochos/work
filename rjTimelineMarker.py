...
The markers are stored in the maya file.
                          ...
                          H O T K E Y:
The hotkey function can be used to setup hotkeys to manage the timeline markers. 
There are three options, this is to either add, remove or clear the markers. 
...
def mayaToQT( name ):
    ...
    if ptr is not None:     return shiboken.wrapInstance( long( ptr ), QWidget )
...
def hotkey( action ):
    ...
    if not "markers" in globals().keys():    return
    elif action == "add":                    markers.add()
    elif action == "remove":                 markers.remove()
    elif action == "clear":                  markers.clear()
